...
def crawling():
    ...
    for i, key in enumerate(issue_list):
      try:
        key_url = "http://jira.duzon.com:8080/browse/" + key.get_text()
        ...
        if len(issue_list) > 1:
          printProgress(i, len(issue_list) - 1, '제작중:', str(i + 1) + "/" + str(len(issue_list)), 1, 50)
      except:
        errList.append(key.get_text())
...
result = crawling()
print("\nfront 배포리스트(개인, 법인) " + str(len(result)) + "개" + today + '\n')
for res in result:
    print(res)
